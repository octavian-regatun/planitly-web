...
const CalendarDay: React.FC<{ date: Date }> = ({ date }) => {
  ...
  return (
    <td className="flex flex-1 flex-col items-center justify-start overflow-hidden border border-l-0 border-b-0 border-gray-200 text-center last:border-r-0 gap-1">
      <p>{date.getDate()}</p>
      {getEventsQuery.data &&
